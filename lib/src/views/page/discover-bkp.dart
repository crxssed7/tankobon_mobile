  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels == _scrollController.position.maxScrollExtent) {
        offset += 8;
        setState(() {
          /* TankobonClient.getManga(offset: offset); */
        });
      }
    });

    itemResults = FutureBuilder<Results>(
      future: TankobonClient.getManga(offset: offset),
                    ...
                    child: Center(
                      child: TextField(
                        ...
                        onSubmitted: (query) {
                          setState(() {
                            itemResults = FutureBuilder<Results>(
                              future: TankobonClient.getManga(query: query, offset: offset),
                              builder: (context, snapshot) {
                                if (snapshot.hasData) {
                                  return GridView.builder(
                                    controller: _scrollController,
                                    ...
                                    itemBuilder: (_, index) => ItemResult(
                                        snapshot.data!.manga![index]),
                                    itemCount: snapshot.data!.manga?.length,
                                  );
                                } else {
                                  return const Center(
